...
# index.
doc_cat_idx = zip_index(doc_cat, 'category_id', 'category_idx')
doc_ent_idx = zip_index(doc_ent, 'entity_id', 'entity_idx')
doc_top_idx = zip_index(doc_top, 'topic_id', 'topic_idx')
uuid_idx = get_index_map(events, 'uuid', 'uuid_idx')
adv_idx = get_index_map(promo, 'advertiser_id', 'advertiser_idx')
cam_idx = get_index_map(promo, 'campaign_id', 'campaign_idx')
src_idx = get_index_map(doc_meta, 'source_id', 'source_idx')
pub_idx = get_index_map(doc_meta, 'publisher_id', 'publisher_idx')
# Sanity checks...
...
def vecsum(r):
    ...
    return LabeledPoint(r['clicked'], vec)
...
print 'Saving model'
print '==============================='
model.save(sc, 'trained_naive_model')
print '==============================='
print 'Validating model'
print '==============================='
valid_set = transform_dataset(clicks_valid).map(lambda r: r['features'])
valid_df = model.predict(valid_set).toDF()
valid_df.write.parquet('valid_result')
print '==============================='
print 'All done'
print '==============================='
